...
<form name="frmSearch" method="post" action="ad_requisicion.php">
	<div class="search-box">
		<p class="search">
			...
			<input class="searchUser" type="text" name="descripcion" id="descripcion" ovalue="<?php echo $name; ?>" placeholder="Búsqueda de Producto">
			<input type="button" name="go" class="btn btn-primary" value="Buscar" onclick="getresult('<?php echo $paginationlink . $page; ?>')">
		...
		<tbody>
			...
			?>
						<tr id="toy-<?php echo $result[$k]["sec"]; ?>">
							<!--<td class="text-center"><input class="" type='checkbox' value="<?php echo $result[$k]["sec"]; ?>" onclick="refresh();"></td>-->
							...
							<td><input type="text" data-cant="<?php echo (int)$result[$k]["cantidad"]; ?>" data-um="<?php echo $result[$k]["um"]; ?>" id="<?php echo (int)$result[$k]["sec"]; ?>" name="<?php echo remove_junk($result[$k]["descripcion"])?>" val="" onchange="AddToHidden(this.name, this.id, this.value, $(this).attr('data-um'), $(this).attr('data-cant'));"></td>
							<td><?php echo (int)$result[$k]["cantidad"]?></td>
						</tr>
			<?php
					}
				}
