      ...
      <div class="panel">
        <div class="jumbotron text-center">
          ...
          <?php
            ...
            $reqPendUsr = 'SELECT no_requisicion FROM requisiciones WHERE solicitante IN (SELECT username FROM users WHERE username LIKE "%' . $e_user["username"] . '%") AND status="2" AND empresa=' . $e_user["working_on"];
            ...
            $reqOcUsr = 'SELECT oc FROM ordenes_de_compra WHERE solicitante IN (SELECT username FROM users WHERE username LIKE "%' . $e_user["username"] . '%") AND status="2" AND empresa="' . $e_empresa["empresa"] . '"';
      ...
      <div class="jumbotron text-center">
        ...
        <?php
          ...
          if ($results->length > 0) {
            ...
            echo money_format("USD: %i", $myObj->output) . " MXN<br>";
          }
          ...
          if ($results->length > 0) {
            ...
            $myObj->output = preg_replace("/[^0-9\.]/", '', $arr[1]);
            //$myJSON = json_encode($myObj);
            echo money_format("EU: %i", $myObj->output) . " MXN<br>";
          }
