                    ...
                    <tbody>
                        ...
                        ?>
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <?php
                                            foreach($empresas AS $empresa) {
                                                if($empresa["id"] == $a_requisicion['empresa']) {
                                                    echo remove_junk($empresa['empresa']);
                                                }
                                            }
                                        ?>
                                    </td>
                                    <td class="text-center"><?php echo remove_junk($a_requisicion['solicitante']); ?></td>
                                    <td class="text-center">
                                        <?php
                                            foreach ($all_proyectos as $proyecto) {
                                                if($proyecto["id"] == $a_requisicion['proyecto']) {
                                    ...
                                    <td class="text-center">
                                        <?php
                                            /*foreach ($all_proveedores as $proveedores) {
                                                if($proveedores["id"] == $a_requisicion['proveedor']) {
                                                    echo remove_junk($proveedores['razon_social']);
                                                }
                                            }*/
                                            echo $a_requisicion['fecha_creacion'];
                                        ?>
                                    </td>
                                    <td class="text-center"><?php echo $a_requisicion['fecha_surtido']?></td>
                                    <td class="text-center">
                                        <?php
                                            $stats = requisicionStatus();
                                            echo $stats[$a_requisicion['status']];
                                        ?>
                                    </td>
                                    <td class="text-center">
                                        <div class="btn-group">
                                            <a href="edit_requisicion.php?id=<?php echo (int)$a_requisicion['id'];?>" class="btn btn-xs btn-warning" data-toggle="tooltip" title="Editar">
                                                <i class="glyphicon glyphicon-pencil"></i>
                                            </a>
                                            <a onclick="return confirm('¿Eliminar la Requisición?');" href="delete_requisicion.php?id=<?php echo (int)$a_requisicion['id'];?>" class="btn btn-xs btn-danger" data-toggle="tooltip" title="Eliminar">
                                                <i class="glyphicon glyphicon-remove"></i>
                                            </a>
                                            <!--<a href="" onclick="printExternal('http://127.0.0.1/inventario/docs/requisiciones/Req-<?php echo $a_requisicion['id'] ?>.pdf')" class="btn btn-xs btn-primary" data-toggle="tooltip" title="Imprimir" onclick="return ConfirmDelete()">-->
                                            <a href="" onclick="printExternal('docs/requisiciones/Req-<?php echo $a_requisicion['id'] ?>.pdf')" class="btn btn-xs btn-primary" data-toggle="tooltip" title="Imprimir">
